...
const writeSummary = async (path, screenshots) => {
    ...
    for (const element of testSuites) {
        ...
        testSuitesElements
            ...
            else if (testSuite.elements.filter((element) => element.name === "skipped")
                .length === 1) {
                status = "skipped";
            }
            else {
            ...
            const test = new TestCase(element.attributes?.name, testSuite.attributes?.classname, testSuite.attributes?.name?.replace("()", ""), testSuite.attributes?.time, errorMessage, status);
...
const groupedTests = (tests) => Object.entries(groupBy(tests, "classname"));
