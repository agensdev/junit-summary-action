...
async function uploadScreenshots(runId, xcresultPath, screenshotsPath = "./screenshots") {
    if (!serviceAccount) {
        throw new Error("Missing Firebase service account.");
    }
    if (!uploadToStorage) {
        throw new Error("In order to upload, you have to set FIREBASE_STORAGE_BUCKET and FIREBASE_SERVICE_ACCOUNT in the environment variables.");
    }
    if (xcresultPath) {
        await getScreenshotsFromXcresult(xcresultPath, screenshotsPath || "./screenshots");
    }
    const images = await glob("./screenshots/**/*.+(png|gif)");
    return await Promise.all(images.map(async (image) => {
        const downloadUrl = await upload(image, `${runId || "local"}/${image.replaceAll("./", "")}`);
        return { image, downloadUrl };
    }));
}
async function getScreenshotsFromXcresult(xcresultPath, destinationPath) {
...
//# sourceMappingURL=uploadScreenshots.js.map